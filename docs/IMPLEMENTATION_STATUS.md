...
- Restored the top-level README to challenge-first framing and kept the repository-specific reviewer guidance as an addendum below it
...
- Refined the reviewer-facing design docs so they now distinguish many-session scaling from one-hotspot-quiz scaling, including Redis-backed session routing and realtime ranking discussion
- Added concrete Mermaid flow charts to the reviewer-facing system design using the actual runtime modules and function names for join, accepted-answer fanout, and score or leaderboard visibility
