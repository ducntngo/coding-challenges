...
Active draft. The early automated headless integration harness now exists, covers accepted answer flow plus session-wide score and leaderboard fanout, duplicate rejection, and now also covers closed-phase and wrong-question rejection driven by real internal question progression with transport-visible `session.snapshot` updates. It should deepen further as scoring behavior and richer late-answer paths become real.
