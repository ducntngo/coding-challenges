...
| 5. Implement scoring and leaderboard flow | In progress | Accepted answer handling now emits session-wide score and leaderboard updates to the active connections in the same quiz session, and non-open phases reject answers. Next add explicit current-question context plus richer wrong-question or late rejection behavior without breaking the established interfaces or guard-rail tests. |
...
- add explicit current-question context and richer answer validation or rejection behavior
...
3. Expand the headless integration harness with duplicate, late, and wrong-question scenarios.
4. Keep the new session-wide fanout and closed-phase rejection paths covered as scoring behavior changes.
5. Keep the unit and integration suites separate while both continue to deepen.

## Resume Point

The next session should resume just after the first phase-aware answer-rejection slice behind the stage-3 scaffold.
...
- add explicit current-question context and richer answer validation and rejection behavior
