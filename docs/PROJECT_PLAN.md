...
- add richer answer validation and rejection behavior as question handling deepens
...
4. Keep the new session-wide fanout path covered as scoring behavior changes.
5. Keep the unit and integration suites separate while both continue to deepen.

## Resume Point

The next session should resume just after the first session-wide fanout slice behind the stage-3 scaffold.
